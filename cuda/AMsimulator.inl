...
#else
__device__ __inline__ float AMsimulator(float Af, float Bf, cudaTextureObject_t lut)
{
    uint32_t  at = *(uint32_t *)&Af;
	uint32_t  bt = *(uint32_t *)&Bf;
	// Extracting mantissa bits: bitwise anding
	uint32_t  Amnt = (0x7f0000 & at);
	uint32_t  Bmnt = (0x7f0000 & bt);
    // Approximate Mantissa calculation
    uint32_t Mbm_mantmult = uint32_t(tex1Dfetch<uint32_t>(lut, (int)((Amnt)>>16) | ((Bmnt)>>9)));
    uint32_t is_normalized = (Mbm_mantmult&SIGN_MASK)>>31;
    Mbm_mantmult = Mbm_mantmult&CLEAR_NORMALIZED;
