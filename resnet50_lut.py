...
"""
def identity_block(input_tensor, kernel_size, filters, stage, block, lut_file, AM=False):
    ...
    x = layers.Conv2D(filters1, (1, 1),
            kernel_regularizer=tf.keras.regularizers.l2(WD),name=conv_name_base + '2a',mant_mul_lut=lut_file)(input_tensor) if KERASWRAPPERS else \
    ...
    x = layers.Conv2D(filters2, kernel_size,
                      padding='same',kernel_regularizer=tf.keras.regularizers.l2(WD), name=conv_name_base + '2b',mant_mul_lut=lut_file)(x) if KERASWRAPPERS else \
    ...
    x = layers.Conv2D(filters3, (1, 1),kernel_regularizer=tf.keras.regularizers.l2(WD), name=conv_name_base + '2c',mant_mul_lut=lut_file)(x) if KERASWRAPPERS else \
...
def conv_block(input_tensor,
    ...
    x = layers.Conv2D(filters1, (1, 1), strides=strides,kernel_regularizer=tf.keras.regularizers.l2(WD),
                      name=conv_name_base + '2a',mant_mul_lut=lut_file)(input_tensor) if KERASWRAPPERS else AMCONV(filters1,1,strides=strides[0],name=conv_name_base+'2a')(input_tensor)
    x = layers.BatchNormalization(axis=bn_axis, momentum=MOMENTUM, epsilon=EPSILON ,name=bn_name_base + '2a')(x)
    x = layers.Activation('relu')(x)

    x = layers.Conv2D(filters2, kernel_size, padding='same',kernel_regularizer=tf.keras.regularizers.l2(WD),
                      name=conv_name_base + '2b',mant_mul_lut=lut_file)(x) if KERASWRAPPERS else AMCONV(filters2,kernel_size,padding='SAME',name=conv_name_base+'2b')(x) 
    x = layers.BatchNormalization(axis=bn_axis,momentum=MOMENTUM, epsilon=EPSILON , name=bn_name_base + '2b')(x)
    x = layers.Activation('relu')(x)

    x = layers.Conv2D(filters3, (1, 1),kernel_regularizer=tf.keras.regularizers.l2(WD), name=conv_name_base + '2c',mant_mul_lut=lut_file)(x) if KERASWRAPPERS else AMCONV(filters3,1,name=conv_name_base+'2c')(x)
    x = layers.BatchNormalization(axis=bn_axis,momentum=MOMENTUM, epsilon=EPSILON , name=bn_name_base + '2c')(x)

    shortcut = layers.Conv2D(filters3, (1, 1),kernel_regularizer=tf.keras.regularizers.l2(WD), strides=strides,
                             name=conv_name_base + '1',mant_mul_lut=lut_file)(input_tensor) if KERASWRAPPERS else AMCONV(filters3,1,strides=strides[0],name=conv_name_base+'1')(input_tensor)
...
def conv_block_small(input_tensor,
    ...
    if first:
        ...
        if in_channel == filters2:
            ...
            shortcut = layers.Conv2D(filters2, 1, padding='same', name=conv_name_base + "sc",mant_mul_lut=lut_file)(input_tensor)

    x = layers.Conv2D(filters1, kernel_size, padding='same',kernel_regularizer=tf.keras.regularizers.l2(WD),
            name=conv_name_base + '2a',mant_mul_lut=lut_file)(input_tensor) if KERASWRAPPERS else AMCONV(filters1,kernel_size,padding='SAME',name=conv_name_base+'2b')(input_tensor) 
    x = layers.BatchNormalization(axis=bn_axis,momentum=MOMENTUM, epsilon=EPSILON , name=bn_name_base + '2a')(x)
    x = layers.Activation('relu')(x)
...
def ResNet34(include_top=True,
    ...
    return model
def ResNet50ImageNet(lut_file,include_top=True,
             weights='imagenet',
             input_tensor=None,
             input_shape=None,
             pooling=None,
    ...
    x = layers.Conv2D(64, (7, 7),
                      strides=(2, 2),
                      padding='valid',kernel_regularizer=tf.keras.regularizers.l2(WD),
                      name='conv1',mant_mul_lut=lut_file)(x) if KERASWRAPPERS else AMCONV(64,7,strides=2,name='conv1')(x)
    x = layers.BatchNormalization(axis=bn_axis, momentum=MOMENTUM, epsilon=EPSILON ,name='bn_conv1')(x)
    x = layers.Activation('relu')(x)
    x = layers.MaxPooling2D((3, 3), strides=(2, 2))(x)

    x = conv_block(x, 3, [64, 64, 256], stage=2, block='a', strides=(1, 1), AM=AM ,lut_file=lut_file)
    x = identity_block(x, 3, [64, 64, 256], stage=2, block='b', AM=AM,lut_file=lut_file)
    x = identity_block(x, 3, [64, 64, 256], stage=2, block='c', AM=AM,lut_file=lut_file)

    x = conv_block(x, 3, [128, 128, 512], stage=3, block='a', AM=AM,lut_file=lut_file)
    x = identity_block(x, 3, [128, 128, 512], stage=3, block='b', AM=AM,lut_file=lut_file)
    x = identity_block(x, 3, [128, 128, 512], stage=3, block='c', AM=AM,lut_file=lut_file)
    x = identity_block(x, 3, [128, 128, 512], stage=3, block='d', AM=AM,lut_file=lut_file)

    x = conv_block(x, 3, [256, 256, 1024], stage=4, block='a', AM=AM,lut_file=lut_file)
    x = identity_block(x, 3, [256, 256, 1024], stage=4, block='b', AM=AM,lut_file=lut_file)
    x = identity_block(x, 3, [256, 256, 1024], stage=4, block='c', AM=AM,lut_file=lut_file)
    x = identity_block(x, 3, [256, 256, 1024], stage=4, block='d', AM=AM,lut_file=lut_file)
    x = identity_block(x, 3, [256, 256, 1024], stage=4, block='e', AM=AM,lut_file=lut_file)
    x = identity_block(x, 3, [256, 256, 1024], stage=4, block='f', AM=AM,lut_file=lut_file)

    x = conv_block(x, 3, [512, 512, 2048], stage=5, block='a', AM=AM,lut_file=lut_file)
    x = identity_block(x, 3, [512, 512, 2048], stage=5, block='b', AM=AM,lut_file=lut_file)
    x = identity_block(x, 3, [512, 512, 2048], stage=5, block='c', AM=AM,lut_file=lut_file)
