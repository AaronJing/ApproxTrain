...
grep "Filter gradient im2col" output | grep -Eo '[+-]?([0-9][.][0-9]+)([e][+-][0-9]+)?' | awk '{ SUM += $1;} END { print "Backpropagation filtergradient  Im2Col " SUM }'
grep "Filter gradient gemm" output | grep -Eo '[+-]?([0-9][.][0-9]+)([e][+-][0-9]+)?' | awk '{ SUM += $1;} END { print "Backpropagation filtergradient gemm " SUM }'
